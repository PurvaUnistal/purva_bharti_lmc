...
class _ImageListScreenState extends State<ImageListScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      onWillPop: () async {
     //   return Navigator.pushAndRemoveUntil(context,MaterialPageRoute(builder: (_) =>  CustomerRecords()),(r) => false);
       return Navigator.push(context,MaterialPageRoute(builder: (context) => CustomerRecords()));
                    ...
                    checkLoading
                      ...
                      text: uploadText,
                        ...
                        onTap: () async {
                          if(checkOutBtnBoth) {
                            int count = 0;
                            CustomLoaderDialog.showLoadingDialog(context, _keyLoader);
                            for (int i = 0; i < localList.length; i++) {
                              ...
                              try {
                                var response = await apiIntegration.lmcInstallationImages(installationImagesReqModel);
                  ...
                  child: Padding(
                      ...
                      child: ValueListenableBuilder(
                          ...
                          builder: (context, box,_) {
                            return dataBox.length == 0 ? Center(child: Text("NO RECORDS FOUNDS")) :
                            ListView.builder(
                                shrinkWrap: true,
                                physics: ClampingScrollPhysics(),
                                itemCount: localList.length,
                                itemBuilder: (context, position) {
                                  ImageDataModel getStudent = localList[position];
                                  return Visibility(
                                    visible: true,
                                    child: Card(
                                      elevation: 8,
                                          ...
                                          children: [
                                            Expanded(
                                              child: Column(
                                                ...
                                                children: [
                                                  ListTile(
                                                    title: Text("RECORDS : ${position + 1}",style: TextStyle(fontWeight: FontWeight.bold, color: Colors.blue),),
                                                    visualDensity: VisualDensity(horizontal: 0, vertical: -4),
                                                    trailing: IconButton(
                                                      icon:
                                                      checkLoading
                                                          ? Icon(Icons.sync, color: Colors.red,)
                                                          : Icon(Icons.sync_outlined,color: Colors.green),
                                                      onPressed: () async{
                                                        CustomLoaderDialog.showLoadingDialog(context, _keyLoader);
                                                        if (checkOutBtnBoth) {
                                                          installationImagesReqModel =  InstallationImagesReqModel(
                                                            ...
                                                            lmcId: getStudent.lmcID,
                                                            dmaId: getStudent.dmaID,
                                                            rfcForm: getStudent.image1.toString(),
                                                            workCompletedImage: getStudent.image2.toString(),
                                                            isometricImage: getStudent.image3.toString(),
                                                            pneumaticImage: getStudent.image4.toString(),
                                                          );
                                                          print("installationImagesReqMode-->");
                                                          print(installationImagesReqModel.toJson());
                                                          try {
                                                            var response = await apiIntegration.lmcInstallationImages(installationImagesReqModel);
                                                            print(response.data);
                                                  ...
                                                  ListTile(
                                                    visualDensity: VisualDensity(horizontal: 0, vertical: -4),
                                                    title:Text("BP Number"),
                                                    trailing: Text(getStudent.bpNumber , style: const TextStyle(fontSize: 18),maxLines: 2,overflow: TextOverflow.ellipsis,),
                                                  ),
                                                  Row(
                                                    ...
                                                    children: [
                                                      Row(
                                                        children: [
                                                          getStudent.image1 != null
                                                              ? CircleAvatar(backgroundImage: FileImage(File(getStudent.image1.toString())))
                                                              : Text("No Image"),
                                                          const SizedBox(width: 10),
                                                          getStudent.image2 != null
                                                              ? CircleAvatar(backgroundImage: FileImage(File(getStudent.image2.toString())))
                                                              : Text("No Image"),
                                                          const SizedBox(width: 10),
                                                          getStudent.image3 != null
                                                              ? CircleAvatar(backgroundImage: FileImage(File(getStudent.image3.toString())))
                                                              : Text("No Image"),
                                                          const SizedBox(width: 10),
                                                          getStudent.image4 != null
                                                              ? CircleAvatar(backgroundImage: FileImage(File(getStudent.image4.toString())))
                                                              : Text("No Image"),
                                                        ],
                                                      ),
                                                      checkLoading
  ...
  void buildAlertDialog(String message) {
    SchedulerBinding.instance.addPostFrameCallback((_) => setState(() {
      if (isOffline && !dialogIsVisible) {
        ...
        showDialog(
            barrierDismissible: false,context: ctx,
              ...
              return AlertDialog(
                ...
                content: Column(
                  ...
                  children: <Widget>[
                    ...
                    canProceed
                        ? Text("Check your internet connection before proceeding.",textAlign: TextAlign.center,
                      style: TextStyle(fontSize: 12.0),
                    )
                        : Text("Please! proceed by connecting to a internet connection",textAlign: TextAlign.center,
                      style: TextStyle(
                          fontSize: 12.0,
                          color: Colors.red),
                    ),
