...
class InstallationScreenPage extends State<InstallationScreen>{
  ...
  Future<void> _getFreeMaterialData() async {
    ...
    if(res.statusCode == 200) {
      ...
      materialList = List.generate(dataList.data.length,(i) =>
          MaterialItem(
              value: '0',
  ...
  Future<void> _getMeters() async {
    ...
    if(dataList.success == 200) {
      ...
      meterDataList = List.generate(dataList.data.length,(i) => dataList.data[i],);
      _meterNoList = List.generate(dataList.data.length,(i) => ('${dataList.data[i].serialNumber}'));
      _meterNoIdList = List.generate(dataList.data.length,(i) => '${dataList.data[i].id}',);
      if (!mounted) return;
      setState(() {
  ...
  Future<void>  _getRegulators() async {
    ...
    if(dataList2.success == 200) {
      ...
      _meterNoList2 = List.generate(
          dataList2.data.length,(i) =>('${dataList2.data[i].serialNumber}'));
      _meterNoIdList2 = List.generate(dataList2.data.length,(i) => '${dataList2.data[i].id}',);
      if (!mounted) return;
      setState(() {
  ...
  List<DropdownMenuItem<OptionItem>> reasonArrItems = ([
    DropdownMenuItem(
      value: OptionItem(id:'0',title: 'Select Delay Reason',),
      child: Text('Select Delay Reason'),
    ),
    DropdownMenuItem(
      value: OptionItem(id:'1',title: 'Pipeline not charged',),
      child: Text('Pipeline not charged'),
    ),
    DropdownMenuItem(
      value: OptionItem(id:'2',title: 'Contractor not available',),
      child: Text('Contractor not available'),
    ),
    DropdownMenuItem(
      value: OptionItem(id:'3',title: 'Customer hold',),
      child: Text('Customer hold'),
    ),
    DropdownMenuItem(
      value: OptionItem(id:'4',title: 'Customer unavailable',),
  ...
  Future<void> isRfcList() async {
    var res = await http.get(Uri.parse(GlobalConstants.getRfc,));
   print("getRfc--> " + res.body);
    final decoded = jsonDecode(res.body) as Map;
    decoded.forEach((k,v) {
      rfcList.add(OptionItem(id: k,title: v));
      Map<String, String> map = {'$k': '0',};
  ...
  _uploadImage() async {
    ...
    String url = GlobalConstants.saveLmcInstallation;
      ...
      // if(!(workCompleteController==null || workCompleteController.imagePath.path=='')) {
      //   var pic2 = await http.MultipartFile.fromPath( "work_completed_image", workCompleteController.imagePath.path);
      //   request.files.add(pic2);
      //   print("pic2 ");
      // }
      // if(isometricImgController.imagePath != null) {
      //   if(!(isometricImgController == null || isometricImgController.imagePath.path == '')) {
      //     var pic4 = await http.MultipartFile.fromPath("isometric_image", isometricImgController.imagePath.path);
  ...
  Widget build(BuildContext context) {
    ...
    //  key: key,
      ...
      textChanged: (text) => {currentMeterNo = text,},
      clearOnSubmit: false,
    ...
    _autoCompleteTextView2 = SimpleAutoCompleteTextField(
      ...
      textChanged: (text) => {currentMeterNo2 = text,},
      clearOnSubmit: false,
      textSubmitted: (text) => setState(() {
        if (text != "") {
          //addedMeterNo.clear();
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('LMC Installation',style: AppTextStyle.toolbarHeadline,),),
      body: (!_showProgress)  ? SingleChildScrollView(
        child: Padding(
            ...
            builder: (BuildContext context, StateSetter setState){
              return Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 10,),
                    getDropDown(typeOfNrItems, _typeOfNrc,title: typeOfNrLabel,onChanged:(OptionItem value) {
                      setState(() {
                        _typeOfNrc = value;
                        ...
                        return ListTile(
                          ...
                          title: Row(
                            ...
                            children: [
                              ...
                              SizedBox(width: 5,),
                              (item.name.toLowerCase()).contains('pipe') ? Expanded(
                                flex:3,
                                child:getQtyTextField(
                                  ...
                                  onChanged: (value) {
                                    if (value != null )
                                      _getExtraPipeDetails(value.toString());
                                  },
                                ),
                              ) : Expanded(
                                flex:3,
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: Padding(
                            ...
                            child: Text(meterPhotoLabel,style: AppTextStyle.headline,),),
                        ),
                        SizedBox(height: 15,),
                        Padding(
                          padding: EdgeInsets.all(5),
                          child: Text(meterNoLabel,style: AppTextStyle.headline,),
                        ),
                        _autoCompleteTextView,
                        Padding(
                            padding: const EdgeInsets.all(4.0)
                        ),
                        Text(initialReadingLabel,style: AppTextStyle.headline,),
                        Row(
                          mainAxisSize: MainAxisSize.max,
                          children: [
                            Container(width: 35,margin: EdgeInsets.fromLTRB(0, 0, 1, 0), child: Center(child: getTextMeterFormField(initialReadingController ,fieldText: '0',keyboardType: TextInputType.number,maxLimit: 1))),
                            Container(width: 35,margin: EdgeInsets.fromLTRB(0, 0, 1, 0), child: Center(child: getTextMeterFormField(initialReadingController2,fieldText: '0',keyboardType: TextInputType.number,maxLimit: 1))),
                            Container(width: 35,margin: EdgeInsets.fromLTRB(0, 0, 1, 0), child: Center(child: getTextMeterFormField(initialReadingController3,fieldText: '0',keyboardType: TextInputType.number,maxLimit: 1))),
                        ...
                        Row(
                          ...
                          children: [
                            meterImgController.imagePath!=null
                                ...
                                child:Row(
                                  children: [
                                    Image.file(meterImgController.imagePath, width: 100,height: 100, fit: BoxFit.cover,),
                                  ],
                                )
                            )
                                : Container(
                                child:Row(
                                    children: [
                                      Image.asset('assets/icons/place_holder.png', width: 100,height: 100,fit: BoxFit.cover,)
                                    ]
                                )
                            ),
                            Padding(
                              ...
                              child: Column(
                                  ...
                                  children: [
                                    ElevatedButton(
                                        ...
                                        child: Text(takeMeterPhotoLabel,style: TextStyle(color: Colors.white),),
                                        onPressed: () async {
                    ...
                    getLatLongTextField(
                        tfLatitudeController..text = tfPositional != null ? tfPositional.longitude.toString() : "",
                        hintText: tfLatitudeLabel,
                        focusable: true
                    ),
                    getLatLongTextField(
                        tfLongitudeController..text = tfPositional != null ? tfPositional.latitude.toString() : "",
                    ...
                    Row(
                        ...
                        children: [
                          ElevatedButton(
                              style: ElevatedButton.styleFrom(
                                primary: Theme.of(context).primaryColor,),
                              child: Text(btnTfLocationLabel,style: TextStyle(color: Colors.white),),
                              onPressed: () async {
                                _getCurrentLocation('TF');
                              }
                          ),
                    ...
                    getLatLongTextField(
                        houseLatitudeController..text = housePositional != null ? housePositional.longitude.toString() : "",
                        hintText: houseLatLabel,focusable: true),
                    getLatLongTextField(houseLongitudeController..text = housePositional != null ? housePositional.latitude.toString() : "",
                        hintText: houseLongLabel,focusable: true),
                    Row(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          ElevatedButton(
                              style: ElevatedButton.styleFrom(
                                primary: Theme.of(context).primaryColor,),
                              child: Text(btnHouseLocationLabel,style: TextStyle(color: Colors.white),),
                              onPressed: () async {
                                _getCurrentLocation('');
                              }
                          ),
                      ...
                      children: [
                        DefaultTextStyle(
                          ...
                          child: InkWell(
                            ...
                            onTap: (){
                              conversionDate(context).then((value) =>
                                  setState(() {
                        ...
                        Container(
                          width: MediaQuery. of(context). size. width,
                          color: Colors.grey[200],
                          child: Padding(padding:EdgeInsets.all(10.0),child: Text('Checklist Before RFC',style: AppTextStyle.headline,),),
                        ),
                        SizedBox(height: 15,),
                          ...
                          children: rfcList.map((item) {
                            return CheckboxListTile(
                              ...
                              title: Align(
                                ...
                                child: Text('${item.title}',style: AppTextStyle.textContent,),
                              ),
                              onChanged: (val) {
                                setState(() {
                                  if(val)
                    ...
                    Align(
                      ...
                      child: Padding(
                        ...
                        child: hideButton  ? null :  ElevatedButton(
                            style: ElevatedButton.styleFrom(primary: Theme.of(context).primaryColor,),
                            child: Container(
                              width:200,
                              child: Align(
                                alignment:Alignment.center,
                                child: Text(btnSubmit,style: TextStyle(color: Colors.white),),
                              ),
                            ),
                            onPressed: () async {
                              _uploadImage();
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: SingleChildScrollView(
            child: ListBody(
              children: <Widget>[
                ListTile(title: Text('Gallery',style: AppTextStyle.textTitle,),onTap: (){
                  Navigator.of(context).pop();
                  getImage(controller,ImageSource.gallery).then((value) => setState(() {}));
                }),
                ListTile(title: Text('Camera',style: AppTextStyle.textTitle),onTap: (){
                  Navigator.of(context).pop();
