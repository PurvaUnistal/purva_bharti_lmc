...
class _BPListDataImagePageState extends State<BPListDataImagePage> {
  ...
  Future fetchInstallDoneResModel() async {
    ...
    var response =
        await apiIntegration.installDoneApi(installDoneResModel, pageNumber);
    setState(() {
                            ...
                            ? Expanded(
                                child: ListView.builder(
                                    ...
                                    itemBuilder:
                                      ...
                                      Rows _rows =
                                          searchRowList.elementAt(index);
                                      return Column(
                                        children: [
                                          InkWell(
                                            onTap: () async {
                                              var bpn =
                                                  _rows.bpNumber.toString();
                                              var n = dataBox.length;
                                              bool b = false;
                                              for (int i = 0; i < n; i++) {
                                                var m =
                                                    dataBox.getAt(i).bpNumber;
                                                if (m == bpn) {
                                                  CustomToast.showToast(
                                                      "BP Number Record Already Exist");
                                                  b = true;
                                                  break;
                                                }
                                              }
                                              if (!b) {
                                                SharedPreferences prefsimage =
                                                    await SharedPreferences
                                                        .getInstance();
                                                prefsimage.setString(
                                                    GlobalConstants.bpNumber,
                                                    _rows.bpNumber);
                                                prefsimage.setString(
                                                    GlobalConstants.lmcId,
                                                    _rows.id);
                                                prefsimage.setString(
                                                    GlobalConstants.dmaId,
                                                    _rows.dmaId);
                                                Navigator.push(
                                                    context,
                                                    MaterialPageRoute(
